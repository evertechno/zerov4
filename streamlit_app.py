...
BENCHMARK_SYMBOL = "NIFTY 50" # Default benchmark for advanced ratios
...
if "benchmark_historical_data" not in st.session_state: # NIFTY 50 benchmark data for advanced ratios
...
def add_technical_indicators(df: pd.DataFrame, sma_periods, ema_periods, rsi_window, macd_fast, macd_slow, macd_signal, bb_window, bb_std_dev) -> pd.DataFrame:
    """Adds multiple customizable technical indicators to the DataFrame."""
    ...
    # Clean up NaNs created by indicators for safe downstream use
...
def calculate_performance_metrics(returns_series: pd.Series, risk_free_rate: float = 0.0, benchmark_returns: pd.Series = None) -> dict:
    """Calculates comprehensive performance metrics, including advanced ratios using an optional benchmark."""
    ...
    if (1 + daily_returns_decimal > 0).all() and len(daily_returns_decimal) > 0:
        geometric_mean_daily = np.expm1(np.log1p(daily_returns_decimal).mean())
        annualized_return = ((1 + geometric_mean_daily) ** TRADING_DAYS_PER_YEAR - 1) * 100
    ...
    annualized_downside_std_dev = downside_std_dev * np.sqrt(TRADING_DAYS_PER_YEAR) if not np.isnan(downside_std_dev) else np.nan
    ...
    if benchmark_returns is not None and not benchmark_returns.empty:
        ...
        if len(common_index) > 1:
            ...
            if benchmark_variance > 0:
                ...
                alpha = (expected_asset_return - (risk_free_rate_decimal + beta * (benchmark_annualized_return - risk_free_rate_decimal))) * 100
                
                # Treynor Ratio: (Ra - Rf) / Beta
                treynor_ratio = (expected_asset_return - risk_free_rate_decimal) / beta if beta != 0 else np.nan

    # Use a helper to round non-NaN floats
...
def _calculate_historical_index_value(api_key: str, access_token: str, constituents_df: pd.DataFrame, start_date: datetime.date, end_date: datetime.date, exchange: str = DEFAULT_EXCHANGE) -> pd.DataFrame:
    ...
    for i, row in constituents_df.iterrows():
        symbol = row['symbol']
        progress_bar.progress((i + 1) / len(constituents_df), text=f"Fetching historical data for {symbol}...")
        
        hist_df = get_historical_data_cached(api_key, access_token, symbol, start_date, end_date, "day", exchange)
        
        if "_error" not in hist_df.columns and not hist_df.empty:
            ...
            st.warning(f"Skipping {symbol} for historical calculation. Error: {error_msg}")
...
def generate_factsheet_csv_content(
    ...
    if not factsheet_constituents_df_final.empty:
        ...
        for col in ['Last Price', 'Weighted Price']:
            if col not in const_export_df.columns: const_export_df[col] = np.nan
    ...
    content.append("\n--- Performance Metrics (Includes Alpha/Beta/Sortino/Calmar/Treynor) ---\n")
...
with st.sidebar:
    ...
    if st.session_state["user_session"]:
        ...
        with st.form("supabase_auth_form_logged_out_static_key"):
            ...
            login_submitted = col_auth1.form_submit_button("Login")
            signup_submitted = col_auth2.form_submit_button("Sign Up")
    ...
    if st.session_state["kite_access_token"]:
        ...
        if st.session_state.get("holdings_data") is not None and not st.session_state["holdings_data"].empty:
            with st.expander("Show Holdings"):
                st.dataframe(st.session_state["holdings_data"][['tradingsymbol', 'quantity', 'average_price', 'last_price', 'exchange', 'pnl']])
    else:
...
def render_market_historical_tab(kite_client: KiteConnect | None, api_key: str | None, access_token: str | None):
    ...
    if not kite_client or not api_key or not access_token:
    ...
    with col_market_quote1:
        ...
        if st.button("Get Market Data", key="get_market_data_btn"):
            ...
            else: st.error(f"Market data fetch failed: {ltp_data.get('_error', 'Unknown error')}")
    ...
    with c3:
        interval = st.selectbox("Interval", ["day", "week", "minute", "5minute", "30minute"], index=0, key="hist_interval_selector")

    if st.button("Fetch Data", key="fetch_hist_data_ta_btn", type="primary"):
        with st.spinner(f"Fetching data for {hist_symbol}..."):
            ...
            if "_error" not in df_hist.columns:
                st.session_state["historical_data"] = df_hist
                st.session_state["last_fetched_symbol"] = hist_symbol
                st.success(f"Fetched {len(df_hist)} records.")
            else: st.error(f"Fetch failed: {df_hist.get('_error', ['Unknown error'])[0]}")

    if not st.session_state.get("historical_data", pd.DataFrame()).empty:
        df = st.session_state["historical_data"]
        
        with st.expander("Technical Indicator Settings", expanded=False):
        ...
        if "SMA" in indicators_to_plot:
            for p in sma_periods: fig.add_trace(go.Scatter(x=df_with_ta.index, y=df_with_ta[f'SMA_{p}'], mode='lines', name=f'SMA {p}'), row=1, col=1)
        if "EMA" in indicators_to_plot:
            for p in ema_periods: fig.add_trace(go.Scatter(x=df_with_ta.index, y=df_with_ta[f'EMA_{p}'], mode='lines', name=f'EMA {p}'), row=1, col=1)
        if "Bollinger Bands" in indicators_to_plot:
            fig.add_trace(go.Scatter(x=df_with_ta.index, y=df_with_ta['BB_High'], mode='lines', line=dict(width=0.5, color='gray'), name='BB High'), row=1, col=1)
            fig.add_trace(go.Scatter(x=df_with_ta.index, y=df_with_ta['BB_Low'], mode='lines', line=dict(width=0.5, color='gray'), fill='tonexty', fillcolor='rgba(128,128,128,0.2)', name='BB Low'), row=1, col=1)
...
def render_custom_index_tab(kite_client: KiteConnect | None, supabase_client: Client, api_key: str | None, access_token: str | None):
        ...
        if uploaded_file:
            try:
                ...
                if not required_cols.issubset(set(df_constituents_new.columns)):
                    ...
                    if df_constituents_new.empty: st.error("No valid constituents found.")
                    else:
                        ...
                        st.info(f"Loaded {len(df_constituents_new)} constituents from CSV. Weights normalized.")
            except Exception as e: st.error(f"Error processing CSV: {e}.")
    ...
    if not current_calculated_index_data_df.empty:
        st.subheader("Configure & Calculate Historical Performance")
        ...
        with calc_c3:
            st.markdown("*(Weights already normalized)*")

        if st.button("Calculate Historical Index Values", key="calculate_new_index_btn"):
            if hist_start_date >= hist_end_date:
                ...
                if not index_history_df_new.empty and "_error" not in index_history_df_new.columns:
                    ...
                    st.error(f"Failed to calculate historical index values: {index_history_df_new.get('_error', ['Unknown error'])[0]}")
                    st.session_state["current_calculated_index_history"] = pd.DataFrame()
    ...
    if not current_calculated_index_data_df.empty and not current_calculated_index_history_df.empty:
        # Display details for newly calculated index
        display_single_index_details("Newly Calculated Index", current_calculated_index_data_df, current_calculated_index_history_df, index_id="new_index")
        ...
        if st.button("Save New Index to DB", key="save_new_index_to_db_btn"):
            ...
            else: st.warning("Please enter an index name and ensure you are logged into Supabase.")
    ...
    if st.button("Load My Indexes from DB", key="load_my_indexes_db_btn"):
        try:
            ...
            else: st.session_state["saved_indexes"] = []; st.info("No saved indexes found.")
        except Exception as e: st.error(f"Error loading indexes: {e}")
    
    saved_indexes = st.session_state.get("saved_indexes", [])
    index_names_from_db = [idx['index_name'] for idx in saved_indexes] if saved_indexes else []
    ...
    with col_comp_bench:
        benchmark_symbols_str = st.text_area(
            f"Enter External Benchmark Symbols (comma-separated, {BENCHMARK_SYMBOL} is implicit for ratios)",
    ...
    risk_free_rate = st.number_input("Risk-Free Rate (%) for Ratios (e.g., 6.0 for 6%)", min_value=0.0, max_value=20.0, value=6.0, step=0.1)

    if st.button("Run Multi-Index & Benchmark Comparison", key="run_multi_comparison_btn", type="primary"):
        ...
        # 1. Fetch NIFTY 50 Benchmark for Alpha/Beta Calculation
        with st.spinner(f"Fetching {BENCHMARK_SYMBOL} data for risk analysis..."):
            benchmark_df = get_historical_data_cached(api_key, access_token, BENCHMARK_SYMBOL, comparison_start_date, comparison_end_date, "day", DEFAULT_EXCHANGE)
            if "_error" in benchmark_df.columns:
                st.warning(f"Could not fetch benchmark '{BENCHMARK_SYMBOL}'. Risk ratios (Alpha/Beta/Treynor) will be N/A.")
        ...
        for i, name in enumerate(comparison_items):
            progress_bar.progress((i + 1) / len(comparison_items), text=f"Processing: {name}...")
            
            data_type = "custom_index" if name in selected_custom_indexes_names else "benchmark"
            constituents_df = None
            symbol = None
            exchange = comparison_exchange
            
            if data_type == "custom_index":
                db_index_data = next((idx for idx in saved_indexes if idx['index_name'] == name), None)
            ...
            if "_error" not in normalized_df_result.columns:
                ...
                st.error(f"Error processing {name}: {normalized_df_result.loc[0, '_error']}")
        ...
        if all_normalized_data:
            ...
            if not combined_comparison_df.empty:
                ...
                st.success("Comparison data and comprehensive metrics generated successfully.")
            else:
                st.warning("No common or sufficient data found for comparison.")
        else:
    ...
    if not last_comparison_df.empty:
        ...
        fig_comparison.update_layout(
            title_text="Multi-Index & Benchmark Performance", xaxis_title="Date", yaxis_title="Normalized Value (Base 100)",
    ...
    selected_constituents_for_factsheet = st.multiselect(
        ...
        default=st.session_state["factsheet_selected_constituents_index_names"],
    ...
    if selected_constituents_for_factsheet and "None" not in selected_constituents_for_factsheet:
        ...
        for index_name in selected_constituents_for_factsheet:
            if index_name != "Newly Calculated Index":
    ...
    ai_agent_snippet_input = st.text_area(
        ...
        height=150, key="ai_agent_embed_snippet_input", 
    ...
    with col_factsheet_download_options_1:
        if st.button("Generate & Download Factsheet (CSV)", key="generate_download_factsheet_csv_btn"):
            if not factsheet_constituents_df_final.empty or not factsheet_history_df_final.empty or not last_comparison_df.empty:
                ...
                st.download_button(
                    label="Download CSV Factsheet", data=st.session_state["last_facts_data"],
                    file_name=f"Factsheet_CSV_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", mime="text/csv",
                    key="factsheet_download_button_final_csv"
                )
                st.success("CSV Factsheet generated!")
            else: st.warning("No data available to generate a factsheet.")
    ...
    with col_factsheet_download_options_2:
        if st.button("Generate & Download Factsheet (HTML/PDF)", key="generate_download_factsheet_html_btn"):
            if not factsheet_constituents_df_final.empty or not factsheet_history_df_final.empty or not last_comparison_df.empty:
                ...
                st.download_button(
                    label="Download HTML Factsheet", data=st.session_state["last_factsheet_html_data"],
                    file_name=f"Factsheet_HTML_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html", mime="text/html",
                    key="factsheet_download_button_final_html"
                )
                st.success("HTML Factsheet generated! (Open in browser, then 'Print to PDF')")
            else: st.warning("No data available to generate a factsheet.")
    ...
    if selected_index_to_manage != "--- Select ---":
        ...
        if selected_db_index_data:
            ...
            if loaded_historical_performance_raw:
                ...
                except Exception: loaded_historical_df = pd.DataFrame()
            
            if loaded_historical_df.empty or 'index_value' not in loaded_historical_df.columns:
                st.warning(f"Historical data for '{selected_index_to_manage}' is missing/invalid. Recalculating last 365 days...")
                ...
                else: st.error(f"Failed to recalculate historical data: {recalculated_historical_df.get('_error', 'Unknown error')}")
