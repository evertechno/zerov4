...
st.set_page_config(page_title="Invsion Connect - Compliance & AI Analysis", layout="wide", initial_sidebar_state="expanded")
st.title("Invsion Connect")
st.markdown("A platform for validating investment compliance and AI-powered analysis, with Supabase user login and full data persistence.")
...
defaults = {
    ...
    "breach_alerts": [],
    "sb_user": None,            # Supabase user dict
    "sb_session": None,         # Supabase session
    "sb_client": None,          # Supabase client
    "last_saved_run_id": None   # Supabase analysis_runs.id
}
...
def load_secrets():
    ...
    if not supabase_conf.get("url") or not supabase_conf.get("anon_key"):
        errors.append("Supabase (url, anon_key)")
...
st.session_state["sb_client"] = init_supabase_client(SUPABASE_CREDENTIALS["url"], SUPABASE_CREDENTIALS["anon_key"])

# --- Supabase Auth Helpers ---
def supabase_signup(email: str, password: str):
    try:
        res = st.session_state["sb_client"].auth.sign_up({"email": email, "password": password})
        return res
...
def save_run_to_supabase(run_label: str, payload: dict):
    """
    Saves the entire run payload as a single JSON row into `analysis_runs`.
    Requires `analysis_runs` table with columns: id (bigint), user_id (uuid), created_at (timestamptz), run_label (text), payload (jsonb).
    """
    if not require_supabase_user():
        return None

    user_id = st.session_state["sb_user"].id if st.session_state["sb_user"] else None
    try:
        data = {
            "user_id": user_id,
            "run_label": run_label,
        ...
        if hasattr(res, "data") and res.data:
            ...
            st.success(f"✅ Saved to Supabase (analysis_runs.id = {run_id})")
            return run_id
        else:
            st.warning("Inserted but no data returned. Check RLS / permissions.")
            return None
    except Exception as e:
        st.error(f"Supabase save failed: {e}")
        return None

def dictify_df(df: pd.DataFrame):
...
def download_payload(payload: dict, filename: str):
    b = json.dumps(payload, default=str, ensure_ascii=False, indent=2).encode("utf-8")
    st.download_button("📥 Download Full JSON", b, file_name=filename, mime="application/json", use_container_width=True)
    ...
    for line in rules_text.strip().split('\n'):
        ...
        try:
            ...
            if rule_type == 'STOCK' and len(parts) == 4:
                ...
                matching = next((s for s in sector_weights.index if s.upper() == sector_name), None)
                if matching:
                    actual_value = sector_weights.get(matching, 0.0)
                    details = f"Actual for {matching}: {actual_value:.2f}%"
                else:
...
# --- Sidebar: Supabase Login + Kite Login ---
with st.sidebar:
    st.markdown("### 🔐 Supabase Login")
    if not st.session_state["sb_user"]:
        with st.form("sb_auth_form", clear_on_submit=False):
            ...
            submit_signup = c2.form_submit_button("Sign Up", use_container_width=True)
        if submit_signup and email and password:
        ...
        st.success(f"Logged in as: {st.session_state['sb_user'].email}")
...
# --- Main UI - Tabs for modules (REMOVED Market & Historical) ---
tabs = st.tabs(["💼 Investment Compliance", "🤖 AI-Powered Analysis"])
...
def render_investment_compliance_tab(kite_client, api_key, access_token):
    ...
    st.markdown("Comprehensive compliance validation at portfolio and security level with regulatory oversight. All outputs can be saved to Supabase per-user.")
    ...
    with col1:
        ...
        with st.expander("⚙️ Set Compliance Thresholds", expanded=True):
...
""")
    ...
    # If we have results, show dashboards and allow saving to Supabase
    if not results_df.empty and 'Weight %' in results_df.columns:
        ...
        with analysis_tabs[0]:
            ...
            with dash_cols[0]:
                ...
                others_weight = results_df.nsmallest(max(len(results_df)-15, 0), 'Weight %')['Weight %'].sum()
                plot_data = pd.concat([
                    top_15[['Name', 'Weight %']],
                    pd.DataFrame([{'Name': 'Others', 'Weight %': others_weight}])
                ])
                fig_pie = px.pie(plot_data, values='Weight %', names='Name', title='Portfolio Composition (Top 15 + Others)', hole=0.4)
                st.plotly_chart(fig_pie, use_container_width=True)
            with dash_cols[1]:
                sector_data = results_df.groupby('Industry')['Weight %'].sum().reset_index().sort_values('Weight %', ascending=False)
                fig_sector = px.bar(sector_data.head(10), x='Weight %', y='Industry', orientation='h', title='Top 10 Sector Allocations', color='Weight %', color_continuous_scale='Blues')
        ...
        with analysis_tabs[1]:
            ...
            with breakdown_subtabs[0]:
                ...
                fig_holdings = px.bar(top_20, x='Weight %', y='Name', orientation='h', title='Top 20 Holdings by Weight', color='Industry', hover_data=['Symbol', 'Real-time Value (Rs)'])
                fig_holdings.update_layout(yaxis={'categoryorder': 'total ascending'}, height=600)
                st.plotly_chart(fig_holdings, use_container_width=True)
                st.dataframe(top_20.style.format({'Weight %': '{:.2f}%', 'Real-time Value (Rs)': '₹{:,.2f}', 'LTP': '₹{:,.2f}'}), use_container_width=True)
            ...
            with breakdown_subtabs[1]:
                ...
                with col1:
                    fig_sector_bar = px.bar(sector_analysis.reset_index().head(15), x='Weight %', y='Industry', orientation='h', title='Top 15 Sectors by Weight')
                    fig_sector_bar.update_layout(yaxis={'categoryorder': 'total ascending'})
                    st.plotly_chart(fig_sector_bar, use_container_width=True)
                with col2:
                    fig_sector_count = px.bar(sector_analysis.reset_index().head(15), x='Count', y='Industry', orientation='h', title='Top 15 Sectors by Holdings Count', color='Count', color_continuous_scale='Greens')
                ...
                st.dataframe(sector_analysis.style.format({'Weight %': '{:.2f}%', 'Real-time Value (Rs)': '₹{:,.2f}'}), use_container_width=True)

            with breakdown_subtabs[2]:
                if 'Rating' in results_df.columns:
                    st.markdown("##### Credit Rating Distribution")
                    rating_analysis = results_df.groupby('Rating').agg({'Weight %': 'sum','Symbol': 'count'}).rename(columns={'Symbol': 'Count'}).sort_values('Weight %', ascending=False)
                    col1, col2 = st.columns(2)
                    with col1:
                        fig_rating = px.pie(rating_analysis.reset_index(), values='Weight %', names='Rating', title='Rating Distribution by Weight', hole=0.3)
                        st.plotly_chart(fig_rating, use_container_width=True)
                    with col2:
                        fig_rating_bar = px.bar(rating_analysis.reset_index(), x='Weight %', y='Rating', orientation='h', title='Rating Exposure by Weight', color='Weight %', color_continuous_scale='RdYlGn_r')
            ...
            with breakdown_subtabs[3]:
                if 'Market Cap' in results_df.columns:
                    ...
                    mcap_analysis = results_df.groupby('Market Cap').agg({'Weight %': 'sum','Symbol': 'count'}).rename(columns={'Symbol': 'Count'}).sort_values('Weight %', ascending=False)
                    col1, col2 = st.columns(2)
                    with col1:
                        fig_mcap = px.pie(mcap_analysis.reset_index(), values='Weight %', names='Market Cap', title='Market Cap Distribution', hole=0.3)
            ...
            with breakdown_subtabs[4]:
                if 'Asset Class' in results_df.columns:
                    ...
                    asset_analysis = results_df.groupby('Asset Class').agg({'Weight %': 'sum','Symbol': 'count'}).rename(columns={'Symbol': 'Count'}).sort_values('Weight %', ascending=False)
                    col1, col2 = st.columns(2)
                    with col1:
                        fig_asset = px.pie(asset_analysis.reset_index(), values='Weight %', names='Asset Class', title='Asset Class Distribution', hole=0.3)
            ...
            with treemap_cols[0]:
                if treemap_depth == "Industry → Stock":
                    fig_treemap = px.treemap(results_df, path=[px.Constant("Portfolio"), 'Industry', 'Name'], values='Real-time Value (Rs)', hover_data={'Weight %': ':.2f'}, title='Portfolio Treemap: Industry → Stock')
                else:
                    if 'Rating' in results_df.columns:
                        fig_treemap = px.treemap(results_df, path=[px.Constant("Portfolio"), 'Industry', 'Rating', 'Name'], values='Real-time Value (Rs)', hover_data={'Weight %': ':.2f'}, title='Portfolio Treemap: Industry → Rating → Stock')
                    else:
        ...
        with analysis_tabs[2]:
            ...
            if st.session_state.advanced_metrics:
                ...
                risk_cols[0].metric("Daily VaR (95%)", f"{metrics['var_95'] * 100:.2f}%")
                risk_cols[1].metric("Daily VaR (99%)", f"{metrics['var_99'] * 100:.2f}%")
                risk_cols[2].metric("CVaR (95%)", f"{metrics['cvar_95'] * 100:.2f}%")
                risk_cols[3].metric("Portfolio Volatility", f"{metrics['portfolio_volatility'] * 100:.2f}%" if metrics['portfolio_volatility'] is not None else "N/A")
                st.markdown("#### Performance Metrics")
                perf_cols = st.columns(4)
                perf_cols[0].metric(f"Beta (vs {BENCHMARK_SYMBOL})", f"{metrics['beta']:.3f}" if metrics['beta'] is not None else "N/A")
                perf_cols[1].metric("Alpha (Annualized)", f"{metrics['alpha'] * 100:.2f}%" if metrics['alpha'] is not None else "N/A")
                perf_cols[2].metric("Tracking Error", f"{metrics['tracking_error'] * 100:.2f}%" if metrics['tracking_error'] is not None else "N/A")
                perf_cols[3].metric("Information Ratio", f"{metrics['information_ratio']:.3f}" if metrics['information_ratio'] is not None else "N/A")
                st.markdown("#### Diversification Metrics")
                div_cols = st.columns(3)
                div_cols[0].metric("Sortino Ratio", f"{metrics['sortino_ratio']:.3f}" if metrics['sortino_ratio'] is not None else "N/A")
                div_cols[1].metric("Avg Correlation", f"{metrics['avg_correlation']:.3f}")
                div_cols[2].metric("Diversification Ratio", f"{metrics['diversification_ratio']:.3f}" if metrics['diversification_ratio'] is not None else "N/A")
        ...
        with analysis_tabs[3]:
            ...
            rules_text_current = st.session_state.get("compliance_rules_input", rules_text if 'rules_text' in locals() else "")
            validation_results = parse_and_validate_rules_enhanced(rules_text_current, results_df)

            if not validation_results:
                st.info("Define rules to see validation results.")
            else:
                ...
                with filter_cols[0]:
                    status_filter = st.multiselect("Filter by Status", ["✅ PASS", "❌ FAIL", "⚠️ Invalid", "Error"], default=["✅ PASS", "❌ FAIL", "⚠️ Invalid", "Error"])
                with filter_cols[1]:
                    severity_filter = st.multiselect("Filter by Severity", ["🔴 Critical", "🟠 High", "🟡 Medium", "✅ Compliant"], default=["🔴 Critical", "🟠 High", "🟡 Medium", "✅ Compliant"])
                    ...
                    if res['status'] == "✅ PASS":
                        ...
                        with st.expander(f"{res['status']} {severity_label} | `{res['rule']}`", expanded=True):
                            ...
                            if 'actual_value' in res and 'threshold' in res:
                                ...
                                st.metric("Breach Amount", f"{res['breach_amount']:.2f}%", delta=f"{res['breach_amount']:.2f}%", delta_color="inverse")
                    else:
                ...
                if st.button("📥 Export Validation Report", use_container_width=True):
                    ...
                    st.download_button("Download Validation Report (CSV)", csv, f"validation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv", use_container_width=True)

            # Store validation results in session (for save)
        ...
        with analysis_tabs[4]:
            ...
            if security_df.empty:
                ...
                with filter_cols[0]:
                    compliance_filter = st.multiselect("Stock Limit Status", security_df['Stock Limit Breach'].unique(), default=security_df['Stock Limit Breach'].unique())
                with filter_cols[1]:
                    if 'Concentration Risk' in security_df.columns:
                        risk_filter = st.multiselect("Concentration Risk", security_df['Concentration Risk'].unique(), default=security_df['Concentration Risk'].unique())
                with filter_cols[2]:
                    if 'Rating Compliance' in security_df.columns:
                        rating_filter = st.multiselect("Rating Compliance", security_df['Rating Compliance'].unique(), default=security_df['Rating Compliance'].unique())
                ...
                display_columns = ['Name', 'Symbol', 'Industry', 'Weight %', 'Stock Limit Breach', 'Stock Limit Gap (%)', 'Concentration Risk']
                ...
                styled_df = filtered_df[available_columns].style.apply(highlight_compliance, axis=1).format({'Weight %': '{:.2f}%', 'Stock Limit Gap (%)': '{:.2f}%'})
                ...
                if breach_count > 0:
                    ...
                    for idx, row in breach_df.iterrows():
                        with st.expander(f"🔴 {row['Symbol']} - {row['Name']} ({row['Weight %']:.2f}%)", expanded=False):
                            ...
                            cols[2].metric("Excess", f"{row['Weight %'] - single_stock_limit:.2f}%", delta=f"{row['Weight %'] - single_stock_limit:.2f}%", delta_color="inverse")
                ...
                st.download_button("📥 Export Security-Level Compliance Report", csv, f"security_compliance_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv", use_container_width=True)
        ...
        with analysis_tabs[5]:
            ...
            fig_lorenz.add_trace(go.Scatter(x=sorted_df['Rank'], y=sorted_df['Cumulative Weight %'], mode='lines+markers', name='Actual Portfolio', line=dict(color='blue', width=2)))
            fig_lorenz.add_trace(go.Scatter(x=[0, len(sorted_df)], y=[0, 100], mode='lines', name='Perfect Equality', line=dict(color='red', dash='dash')))
            fig_lorenz.update_layout(title='Portfolio Concentration Curve (Lorenz)', xaxis_title='Number of Holdings (Ranked by Weight)', yaxis_title='Cumulative Weight %', hovermode='x unified', height=500)
            ...
            st.dataframe(sector_concentration.style.format({'Total Weight %': '{:.2f}%', 'Max Single Stock %': '{:.2f}%'}), use_container_width=True)

            st.markdown("---")
            st.markdown("#### Sector vs Market Cap Heatmap")
            if 'Market Cap' in results_df.columns:
                pivot_data = results_df.pivot_table(values='Weight %', index='Industry', columns='Market Cap', aggfunc='sum', fill_value=0)
                fig_heatmap = px.imshow(pivot_data, labels=dict(x="Market Cap", y="Industry", color="Weight %"), title="Portfolio Allocation: Sector vs Market Cap", color_continuous_scale='RdYlGn_r', aspect="auto")
            ...
            # Store concentration analytics in session (for save)
            st.session_state["last_concentration"] = {
                "sorted": dictify_df(sorted_df),
                "benchmarks": {
                    "top_1": float(top_1_weight),
                    "top_3": float(top_3_weight),
                    "top_5": float(top_5_weight),
                    "top_10": float(top_10_weight),
                    "top_20": float(top_20_weight)
                },
                "sector_concentration": dictify_df(sector_concentration.reset_index()),
            }
        ...
        with analysis_tabs[6]:
                ...
                if st.button("📊 Generate Full Report", type="primary", use_container_width=True):
                    with st.spinner("Generating comprehensive report..."):
                        ...
                        if "Holdings Detail" in include_sections:
                            report_data['Holdings Detail'] = results_df[['Name', 'Symbol', 'Industry', 'Quantity', 'LTP', 'Real-time Value (Rs)', 'Weight %']]
                        ...
                        if "Risk Metrics" in include_sections and st.session_state.advanced_metrics:
                            ...
                            risk_data = pd.DataFrame([{
                                ...
                                'Value': f"{v:.6f}" if v is not None else "N/A"
                            } for k, v in metrics.items()])
                            report_data['Risk Metrics'] = risk_data

                        if "Compliance Validation" in include_sections:
                            validation_results = st.session_state.get("last_validation_results", [])
                        ...
                        if report_format == "Excel":
                            ...
                            output.seek(0)
                            st.download_button("📥 Download Excel Report", output, f"portfolio_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
                        else:
                            combined_df = pd.concat([df_sheet.assign(Section=name) for name, df_sheet in report_data.items()], ignore_index=True)
                            csv = combined_df.to_csv(index=False).encode('utf-8')
                            st.download_button("📥 Download CSV Report", csv, f"portfolio_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv", use_container_width=True)
            ...
            if 'Market Cap' in display_df.columns: column_order.insert(3, 'Market Cap')
            display_columns = [col for col in column_order if col in display_df.columns]
            st.dataframe(display_df[display_columns].style.format(format_dict), use_container_width=True, height=500)

        # TAB 8: Save / Export (Supabase full JSON)
        with analysis_tabs[7]:
            st.subheader("💾 Persist Everything (Per-User)")
            run_label = st.text_input("Run label (optional)", value=f"run_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
            # Build an exhaustive payload covering all datapoints
            payload = {
                "created_at": datetime.utcnow().isoformat(),
                "user_email": st.session_state["sb_user"].email if st.session_state["sb_user"] else None,
                "thresholds": {
                    "single_stock_limit": float(st.session_state.get("single_stock_limit", 10.0)) if "single_stock_limit" in st.session_state else None,
                    "single_sector_limit": float(st.session_state.get("single_sector_limit", 25.0)) if "single_sector_limit" in st.session_state else None,
                },
                "portfolio": dictify_df(results_df),
                "breaches": st.session_state.get("breach_alerts", []),
                "security_level_compliance": dictify_df(st.session_state.get("security_level_compliance")),
                "rule_validations": st.session_state.get("last_validation_results", []),
                "advanced_metrics": st.session_state.get("advanced_metrics", None),
                "concentration": st.session_state.get("last_concentration", {}),
                "ai_analysis": st.session_state.get("ai_analysis_response", None),
                "meta": {
                    "kite_authenticated": bool(st.session_state.get("kite_access_token")),
            ...
            with save_cols[0]:
                if st.button("🟩 Save to Supabase (analysis_runs)", type="primary", use_container_width=True):
                    rid = save_run_to_supabase(run_label.strip() or None, payload)
                    if rid:
                        st.info(f"Saved analysis run ID: {rid}")
            with save_cols[1]:
                download_payload(payload, f"analysis_run_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")

# ------------------------ AI ANALYSIS TAB ------------------------
def render_ai_analysis_tab(kite_client):
    st.header("🤖 AI-Powered Compliance Analysis (with Google Gemini)")
    st.markdown("Analyze your portfolio against scheme documents (SID/KIM) and general regulatory guidelines using AI. Results are saved with your run if you click Save.")
    ...
    st.info("💡 This tool uses AI for analysis. The output is for informational purposes and is not financial or legal advice.", icon="ℹ️")

    col1, col2 = st.columns([2, 1])
    with col1:
        uploaded_files = st.file_uploader("📄 Upload Scheme Documents (SID, KIM, Investment Policy, etc.)", type=["pdf", "txt"], accept_multiple_files=True, help="Upload regulatory documents for AI-powered compliance analysis")
    with col2:
        st.markdown("**Analysis Options**")
        analysis_depth = st.select_slider("Analysis Depth", options=["Quick", "Standard", "Comprehensive"], value="Standard")
